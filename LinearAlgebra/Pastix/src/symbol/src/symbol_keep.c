...
{
  INT                 levfmax;                    /* Maximum level of fill     */
  INT                 nupdmax;                    /* Maximum number of updates */
  INT                 ctrimax;
  INT                 ctromax;
  INT                 hghtmax;
  INT                 bloknum;
  ...
  if (memAllocGroup ((void **)
                     &keepptr->keeptab, (size_t) (symbptr->bloknbr * sizeof (byte)),
...
void *                      dataptr)              /*+ Parameters for addition function +*/
{
  ...
  byte * restrict           keeptax;
  const SymbolBlok *        bloktax;
  INT                       cblknum;
  INT                       bloknum;
  ...
  for (cblknum = 0; cblknum < symbptr->cblknbr; cblknum ++) {
    ...
    colsiz = (double) (symbptr->cblktab[cblknum].lcolnum - symbptr->cblktab[cblknum].fcolnum + 1); /* Get number of columns */
  ...
  for (cblknum = symbptr->baseval; cblknum < symbptr->baseval + symbptr->cblknbr - 1; cblknum ++) { /* Update pass (+ 3-loop pass, with forward contributions) */
    ...
    for (bloknum = cblktax[cblknum].bloknum + 1; ; bloknum ++) { /* For all existing off-diagonal blocks */
      ...
      do {
        ...
        for ( ; ; ) {
          INT                 nupdval;            /* Number of updates of block(s) to be created */
...
{
  byte * restrict           keeptax;              /* Based access to keep block flag array */
  SymbolBlok *              bloktax;
  INT                       cblknum;
  INT                       bloknew;
...
const INT                   datanbr,              /*+ Number of data fields     +*/
