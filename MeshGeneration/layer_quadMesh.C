...
quadMesh::quadMesh(std::string ss) : gBasis(Square4)
{
  ...
  nelem = nx*(ny-1); 
  nodes = ((nx+1)*(ny+1));
  nbelem= 4*nx+2*(ny-1); //top+bot + side*2 + interface*2
...
//      	<< std::endl;
  ...
  // std::cout[botom,rhs,top,lhs,botI,topI] = ["
  //	    << bcType[0] << bcType[1] << bcType[2] << bcType[3] << bcType[4] << bcType[5] <<std::endl;
...
//loop order is bot->right->top->left->interface_bot->interface_top
...
bgelement quadMesh::intToBge(int n) const
{
 ...
 else if(n < nex + ney - 1) //rhs top
   {
     ee.type = bcType[1];
     ee.nodes[0] = ((n-nex)+2) * (nex+1) - 1;
     ee.nodes[1] = ee.nodes[0] + (nex+1);

     ee.xx[0] = svector<2>(xmax, ymin + (n-nex+1)*hy);
     ee.xx[1] = ee.xx[0] + svector<2>(0.0, hy);
   }


 else if(n < 2*nex+ney-1)  // top
  {
    ee.type = bcType[2];

    ee.nodes[1] = (nex+1)*ney + (n-nex-ney+1);
    ee.nodes[0] = ee.nodes[1] + 1;

    ee.xx[1] = svector<2>(xmin + (n-nex-ney+1) * hx, ymax);
    ee.xx[0] = ee.xx[1] + svector<2>(hx  , 0.0);
  }

 else if(n < 2*nex + ney-1 + (ney-1)/2) // lhs bot
   {
    ee.type = bcType[3];

    ee.nodes[1] = (n-2*nex-ney+1) * (nex+1);
    ee.nodes[0] = ee.nodes[1] +   (nex+1);

    ee.xx[1] = svector<2>(xmin, ymin + (n-2*nex-ney+1)*hy);
    ee.xx[0] = ee.xx[1] + svector<2>(0.0, hy);
  }
 else if(n < 2*nex + 2*ney-2) // lhs top
   {
    ee.type = bcType[3];

    ee.nodes[1] = (n-2*nex-ney+2) * (nex+1);
    ee.nodes[0] = ee.nodes[1] +   (nex+1);
 ...
 else if(n < 3*nex + 2*ney-2) // interface of bot
   {
    ee.type = bcType[4];

    ee.nodes[1] = (n-2*nex-2*ney+2) + (nex+1)*(ney-1)/2;
    ee.nodes[0] = ee.nodes[1] + 1;

    ee.xx[1] = svector<2>( xmin+(n-2*nex-2*ney+2)*hx, -hy/2.0);
    ee.xx[0] = ee.xx[1] + svector<2>(hx,0.0);
  }
 else if(n < 4*nex + 2*ney-2) // interface of top
   {
    ee.type = bcType[5];

    ee.nodes[0] = (n-3*nex-2*ney+2 + (nex+1)*(ney+1)/2);
    ee.nodes[1] = ee.nodes[0]+1;

    ee.xx[0] = svector<2>(xmin+(n-3*nex-2*ney+2)*hx, hy/2.0);
    ee.xx[1] = ee.xx[0] + svector<2>(hx,0.0);
  }
