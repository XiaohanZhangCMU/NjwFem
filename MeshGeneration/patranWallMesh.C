...
public:
  ...
  int getNx(){return layerElms.size()/slip_planes ;}
...
private:
  ...
  std::map<int,int> layerElms; //<global eleID, layerElm id (slip_pl + idx*10)>
...
{
  //std::cout<<"getNodeByElm = "<<tag<<std::endl;
  ...
  if(it == layerElms.end() )
    ...
    {
      //std::cerr<<"found tag = "<<tag<<";layerElms[tag] = "<<layerElms[tag]<<std::endl;
...
double patranMesh::setLayerElems(int layer, double layer_top, double layer_bot, vector<double>& imagNodes)
{
  ...
  for(int ind_ys = 0;ind_ys<slip_planes;ind_ys++)
      ...
      for(it = elms.begin();it!=elms.end();it++)
	{
	  ...
	  if(fabs(v[1] - ys[ind_ys])<0.001)  //0.001 is an tolerance
	    {	      
	      std::pair<int,int> p = std::make_pair(it->first,ncnt*10+ind_ys);
	      layerElms.insert(p); 
	      //layerElms.push_back(p);
	      //imagNodes.push_back(v[0]);
	      //std::cout<<"v[1] = "<<v[1] <<"; ys["<<ind_ys<<"] = "<<ys[ind_ys]<<std::endl;
	      ncnt++;
	      sz++;
	    }
	}
  ...
  std::cout<<"layerElms.size() = "<<layerElms.size()<<std::endl;
  map<int,int>::iterator itm;
  ncnt = 0;
  // for(itm = layerElms.begin();itm!=layerElms.end();itm++)
  //   {
  //     std::cout<<"layerElms["<<ncnt<<"] = " <<itm->first<<"  "<<itm->second<<std::endl;
  //     ncnt++;
  //   }

  int nElmsOnEachSlipPlane = layerElms.size()/slip_planes;
  std::cout<<"nElmsOnEachSlipPlane = " <<  nElmsOnEachSlipPlane <<"; n slip_planes = "<<slip_planes<<std::endl;
  
  assert(layerElms.size()%slip_planes == 0);
  ...
  dx_min = (max[0]-min[0])/(double)nElmsOnEachSlipPlane;
  std::cerr<<"dx_min = " <<dx_min<<"nElmsOnEachSlipPlane = "<<nElmsOnEachSlipPlane <<"max[0] = "<<max[0]<<"min[0] = "<<min[0]<<std::endl;
  for(int ind_ys = 0;ind_ys<slip_planes;ind_ys++){
    for(int i = 0;i<nElmsOnEachSlipPlane;i++)
      {
  	if(i == 0)
  ...
  // 	{
  // 	  layerElms.insert(std::pair<int,pair<int,double> >(sortedLayerElm[j*nElmsOnEachSlipPlane+i].first,std::pair<int,double>(i, imagNodes[i]) ));
  // 	}
  //   }
  ...
  // for(unsigned i = 0;i<layerElms.size();i++)
  //   {
  //     dump<<i<<"   " <<layerElms[i]<<std::endl;
  //   }
