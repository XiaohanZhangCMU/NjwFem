...
// typedef GeometricElement<2, 4> gelement; //GeometricElement<Ndim, Nchi>
// typedef BoundaryGelement<1, 2> bgelement; //BoundaryGelement<Ndim, Nchi>

class unoMesh
{
public:
  unoMesh(std::string ss);

  unoMesh(const unoMesh &mm) 
  {
    std::cerr << "unoMesh(): don't copy a unoMesh\n";
    throw("copying a unoMesh");
  }
    
  unoMesh() 
  {
    std::cerr << "unoMesh(): Default constructor called, nyi\n";
    throw("bad unoMesh constructor");
  }
  ...
  void setMesh(std::vector<double> v);
...
private:
  ...
  map<std::string, vector<int> > elm_lib;//eleTyp -> vector<number of nodes in element, number of faces, number of nodes on face, dimension>
...
// vector<double> unoMesh::params()
// {
//   double vals[] = {min[0],min[1],max[0],max[1],cc[0],cc[1]}; 
//   return vector<double>(vals, vals + sizeof(vals) / sizeof(double) );
// }

unoMesh::unoMesh(std::string ss) : gBasis(Square4) //gBasis(Cube8)
{ 
  
}

void unoMesh::setMesh(std::vector<double> vv)
{
...
double unoMesh::setLayerElems(int layer, double layer_top, double layer_bot, vector<double>& imagNodes)
{
  double dx_min = 1e50;
  ...
  for(it = elms.begin();it!=elms.end();it++)
    {
      ...
      if(v[1] >= layer_bot && v[1]<= layer_top)
	{
  ...
  //   {  
  //   }  
  assert(sortedLayerElm.size()%layer == 0);
  int szImagNodes = sortedLayerElm.size()/layer;
  std::cout<<"szImagNodes = " << szImagNodes<<"; layer = "<<layer<<std::endl;
  ...
  for(int i = 0;i<szImagNodes;i++)
	...
	{
	  val+=(sortedLayerElm[j*szImagNodes+i].second)[0];
	}
      val/=(double)layer;
      for(int j = 0;j<layer;j++)
	{
	  layerElms.insert(std::pair<int,pair<int,double> >(sortedLayerElm[j*szImagNodes+i].first,std::pair<int,double>(i, val) ));
	}
      imagNodes.push_back(val);
    }
  ...
  for(int i = 1;i<(int)imagNodes.size();i++)
...
//for ractangular stripped layer only return smallest spacing of imagNodes
// double unoMesh::setLayerElems(int layer, double layer_top, double layer_bot, vector<double>& imagNodes)
// {
//   double tolerance = 1e-3; //any points has distance bigger than this implies they are two points
//   double dx_min = 1e50;

//   int szImagNodes = imagNodes.size();
//   assert(szImagNodes==0);
...
//       if(fabs(v[1]-cc[1])<= tolerance)
// 	{
// 	  szImagNodes = addtoLayerElms(it->first, v[0], tolerance, imagNodes);  
// 	}
//     }
...
//   std::sort(imagNodes.begin(),imagNodes.end()); assert(*imagNodes.begin() <= *imagNodes.end());

//   map<int,pair<int,double> > :: iterator mit;

//   for(int i = 0;i<(int)imagNodes.size();i++)
//     {
//       int cnt = 0;

//       if(i>0)
// 	{
// 	  if(dx_min>(imagNodes[i]-imagNodes[i-1]))  dx_min = imagNodes[i]-imagNodes[i-1];
// 	}

//       for(mit = layerElms.begin();mit!=layerElms.end();mit++)
// 	{
	  
// 	  if(fabs(mit->second.second - imagNodes[i])<2*tolerance)
// 	    {
// 	      mit->second.first = i;
// 	      cnt++;
// 	    }
// 	  if(fabs(mit->second.second - imagNodes[i])>2*tolerance && fabs(mit->second.second - imagNodes[i])<10*tolerance)
// 	    {
// 	      std::cout<<"mit->first = "<<mit->first<<"; mit->second.second = "<<mit->second.second<<"; imagNodes["<<i<< "] = "<<imagNodes[i]<<"fabs(mit->second.second - imagNodes[i])"<<fabs(mit->second.second - imagNodes[i])<<std::endl;
// 	    }
// 	}

//        std::cout<<"layerElms.size = "<<layerElms.size()<<"; cnt = "<<cnt <<"layer = "<<layer <<"imagNodes["<<i<<"] = "<<imagNodes[i]<<std::endl;
//       assert (cnt == layer);
//     }
...
// int unoMesh::addtoLayerElms(int tag, double x, double tolerance, vector<double>& imagNodes)
// {
//   //std::cout<<"tag = "<<tag<<std::endl;
//   //std::cout<<"x = "<<x<<std::endl;
//   map<int,pair<int,double> > :: iterator it;
//   int cnt = -1; //record last index of 1d node
//   for(it = layerElms.begin();it!=layerElms.end();it++)
//     {
//       //std::cout<<"get in here."<<std::endl;
//       if(it->second.first>cnt){
// 	cnt = it->second.first;
// 	//std::cout<<"it->second.first = "<<it->second.first<<"; cnt = "<<cnt<<std::endl;
//       }
//       if(fabs(x - it->second.second)<tolerance)
// 	{
// 	  //std::cout<<"it->second.second = "<<it->second.second<<std::endl;
// 	  pair<int,double> p = it->second;
// 	  layerElms.insert(pair<int,pair<int,double> >(tag,p));
// 	  return cnt;
// 	}
//     }
//   layerElms.insert(pair<int, pair<int,double> >(tag, pair<int, double>(++cnt, x) ));
//   imagNodes.push_back(x);
//   //std::cout<<"cnt = "<<cnt<<" imagNodes.size() = "<<imagNodes.size()<<std::endl;
//   assert(imagNodes.size() == (unsigned)++cnt);
//   return cnt;
// }

//give center coordinates of a element with tag = I

vector<double> unoMesh::center_elem(int tag)
{
...
void unoMesh::construct_elm_lib()
{
  //Quad4  
  vector<int> myvector1(2);
  myvector1[0] = 4; myvector1[1] = 2;
  elm_lib["Quad4"] = myvector1;
  ...
  //Hex8
  vector<int> myvector2(2);
  myvector2[0] = 8; myvector2[1] = 3;
  elm_lib["Hex8"] = myvector2;
}

int unoMesh::input(std::string rpt)
{
...
void unoMesh::setBoundaryElems()
{
  ...
  for(it=topology.begin();it!=topology.end();it++)
    {
      if((++elemID)%500==0) std::cout<<"ELEM ID = "<<elemID<<std::endl;
...
void
unoMesh::addto(set<vector<int> > pool, vector<int> f)
{

}

void unoMesh::print(set<int> s)
{
  set<int>::iterator it;
...
void unoMesh::swap(vector<int>& v, int i, int j)
{
  double swapV;
  swapV = v[i];
  v[i] = v[j];
...
std::string unoMesh::convertInt(int number)
{
...
void unoMesh::info()
{
  ...
  // std::cout << "unoMesh::info(): patranrilateral mesh" << std::endl;
...
void unoMesh::locate(const svector<nDim>& xx, gelement& ee, svector<nDim>& xi) const
{
  throw("not yet implemented");
}


unoMesh::geiterator::geiterator() {
  loc = 0;
  gm = NULL;
}

unoMesh::geiterator::geiterator(int l, unoMesh * g) {
  loc = l;
  gm = g;
}

void unoMesh::geiterator::operator++() {
  (*this)++;
}

void unoMesh::geiterator::operator++(int) {
  loc++;
}

unoMesh::geiterator& unoMesh::geiterator::operator=(const geiterator & it) {
  loc = it.loc;
  gm = it.gm;
  return *this;
}

bool unoMesh::geiterator::operator==(const geiterator & it) {
...
gelement unoMesh::geiterator::operator*() {
...
unoMesh::geiterator unoMesh::gebegin() {
  return geiterator(0, this);
}

unoMesh::geiterator unoMesh::geend() {
  return geiterator(nElms, this);
}

unoMesh::bgeiterator::bgeiterator() {
  loc = 0;
  gm = NULL;
}

unoMesh::bgeiterator::bgeiterator(int l, unoMesh * g) {
  loc = l;
  gm = g;
}

void unoMesh::bgeiterator::operator++() {
  (*this)++;
}

void unoMesh::bgeiterator::operator++(int) {
  loc++;
}

unoMesh::bgeiterator& unoMesh::bgeiterator::operator=(const bgeiterator & it) {
  loc = it.loc;
  gm = it.gm;
  return *this;
}

bool unoMesh::bgeiterator::operator==(const bgeiterator & it) {
...
bgelement unoMesh::bgeiterator::operator*() {
...
unoMesh::bgeiterator unoMesh::bgebegin() {
  return bgeiterator(0,this);
}

unoMesh::bgeiterator unoMesh::bgeend() {
  return bgeiterator(nbelem,this);
}


gelement unoMesh::intToGe(int n)
{
  gelement ee;
  if((n < 0) || (n >= nElms))
    {
...
bgelement unoMesh::intToBge(int n)
{
  if((n < 0) || (n >= nbelem))
    {
