...
public:
  ...
  int getSlipPlanes() const {return getLayer();}

  std::vector<int> getIndexLayers() const {return IndexLayers;}
...
svector<2> quadMesh::center_elem(int tag)
{
  svector<2> xx;
...
//layer is the total number of slip planes
double quadMesh:: setLayerElems(int layer, double layer_top, double layer_bot, vector<double>& imagNodes )
{
  this->layer = layer;
  double dx = (xmax-xmin)/nex;
  for(int slp = 0;slp<layer;slp++)
    for(int i = 0;i<nex;i++)
      {
	if(i==0)
	  imagNodes.push_back( xmin+dx/2.0);
	else
	  imagNodes.push_back(imagNodes[i-1]+dx);
      }

  //distribute `layer` slip planes evenly across y direction
  //IndexLayers[i] =b : the i th slip plane lies in the center of b th row of elements 
  assert(ney>layer+1);
  int space = ney/(layer+1);
  IndexLayers.push_back(space);
  for (int slp = 1;slp<layer;slp++)    
    IndexLayers.push_back(IndexLayers[slp-1]+space);    

  return dx;
}

//return -1 if quad element 'tag' deos not contains a 1d node,
// otherwise the index of 1d node in imageNodes[]                        
int quadMesh::getNodeByElm(int tag)
{
  int yindex = tag/nex;
  for (int i = 0;i<layer;i++)
    if (IndexLayers[i] == yindex) 
      return nex*i + tag -IndexLayers[i]*nex;    
  return -1;
}
