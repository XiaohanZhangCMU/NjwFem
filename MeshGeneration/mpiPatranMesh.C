...
double patranMesh::setLayerElems(int layer, double layer_top, double layer_bot, vector<double>& imagNodes)
{
  ...
  for(int i = 0;i<szImagNodes;i++)
	...
	{
	  layerElms.insert(std::pair<int,pair<int,double> >(sortedLayerElm[j*szImagNodes+i].first,std::pair<int,double>(i, imagNodes[i]) ));
	}
    }
...
// 	}
//       assert(szImagNodes%2!=0);
//       if(i == szImagNodes/2)
// 	imagNodes.push_back(0.0);
//       else
...
// int patranMesh::addtoLayerElms(int tag, double x, double tolerance, vector<double>& imagNodes)
// {
...
void patranMesh::setBoundaryElems(int rb)
{
  ...
  if(rb == 1)
    ...
    {
      std::cout<<"SET BOU1NDARY ELEMENTS"<<std::endl;
...
bgelement patranMesh::intToBge(int n)
{
  ...
  //for test dynamic bulk
 
  // if(fabs(be.xx[0][1]-max[1])<1e-4 && fabs(be.xx[1][1]-max[1])<1e-4)
  //   {    
  //     // std::cout<<"max[1] = "<<max[1]<<" xx[0][1] = "<<be.xx[0][1]<<" xx[1][1] = "<<be.xx[1][1]<<std::endl;
  //     be.type = 1;
  //   }
  // else    
  //   be.type = 0; //1: neumann. 0: dirichlet

  //for PS 
  be.type = 1;

  //for super-sonic dislocation
  //be.type = 0;
